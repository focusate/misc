...
class TestContextHelpers(common.TestOdootilCommon):
    ...
    def test_05_get_active_records_1(self):
        ...
        record = self.Odootil.with_context(
            active_id=self.partner_1.id,
    ...
    def test_06_get_active_records_2(self):
        ...
        records = self.Odootil.with_context(**ctx).get_active_records()
        self.assertEqual(
            set(records.ids), set((self.partner_1 | self.partner_2).ids))
        # Force only single record.
        with self.assertRaises(ValidationError):
            self.Odootil.with_context(**ctx).get_active_records(single=True)
        # Pass custom message for single=True exception.
        try:
            self.Odootil.with_context(**ctx).get_active_records(
                single=True, msg='my_custom_message')
        except ValidationError as e:
            self.assertEqual(e.name, 'my_custom_message')
    ...
    def test_08_force_with_context(self):
        ...
        self.Odootil.force_with_context(Partner)
        self.assertEqual(Partner._context, {'t1': True})
        # Passing new key for context.
        self.Odootil.force_with_context(Partner, t2=True)
        self.assertEqual(Partner._context, {'t1': True, 't2': True})
        # Update existing key.
        self.Odootil.force_with_context(Partner, t1=False)
